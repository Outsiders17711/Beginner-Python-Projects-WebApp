...
def brighten(image, factor):
    ...
    (
        ...
        y_pixels,
        num_channels,
    ) = image.array.shape  # represents x, y pixels of image, # channels (R, G, B)
    new_im = Image(
        x_pixels=x_pixels, y_pixels=y_pixels, num_channels=num_channels
        ...
        for y in range(y_pixels):
            for c in range(num_channels):
                ...
                for x_i in range(
                    ...
                    min(new_im.x_pixels - 1, x + neighbor_range) + 1,
                ):
                    for y_i in range(
                        max(0, y - neighbor_range),
                        min(new_im.y_pixels - 1, y + neighbor_range) + 1,
                    ):
...
def combine_images(image1, image2):
    ...
    (
        ...
        y_pixels,
        num_channels,
    ) = image1.array.shape  # represents x, y pixels of image, # channels (R, G, B)
    new_im = Image(
        x_pixels=x_pixels, y_pixels=y_pixels, num_channels=num_channels
    )  # making a new array to copy values to!
    for x in range(x_pixels):
